        integer *4 itime,ntime,idum,nparti,i,k,tcorr,j
        parameter(ntime=300000,k=50,tcorr=50000)
        real *8 fx_vis(k),fy_vis(k),eta,dt,
     &  vx(k),x(k),vy(k),y(k),f_int(k),
     &  alpha,ran2,gasdev,temp_eta,D,T,beta,k_spr,Fintsp,Fintspx,
     &  fx_int(k),fy_int(k),l0,Fintspy,
     &  sum1,sum2,end_avg,end2_avg,r1,rnparti,rr
        ...
        open(unit=20,file="avg.d",status="unknown")
        ...
        do itime=1,ntime
          ...
          if(itime .gt. tcorr)then
          r1=sqrt((x(1)**2)+(y(1)**2))
          rnparti=sqrt((x(nparti)**2)+(y(nparti)**2))
          sum1=sum1+(rnparti-r1)
          sum2=sum2+((rnparti-r1)**2)
          endif
        enddo
        end_avg=sum1/(ntime-tcorr)
        end2_avg=sqrt(sum2/(ntime-tcorr))
        write(20,*)nparti,end_avg,end2_avg        
        enddo
