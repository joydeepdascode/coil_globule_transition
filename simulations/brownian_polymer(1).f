        implicit none
        integer *4 itime,ntime,idum,npart,i,j,itimeqb,k
        parameter(ntime=4000000,k=100,itimeqb=1000000)
        real *8 T,D,beta,alpha,T_eta,k_spr,rij,l0,ran2,gasdev,
     &  fx_vis,fy_vis,x(k),y(k),vx(k),vy(k),
     ...
     &  x2sumee,y2sumee,dt,vx2avg,vy2avg,x2avg(k),etax,etay
        ...
        do i=1,npart
                   ...
                   j=i+1
                        ...
                        fx_int(j)= fx_int(j) - fx_spr
                        fy_int(j)= fy_int(j) - fy_spr
                        !write(*,*) fx_int(i)
2                       fx_vis= - alpha*vx(i)
                        fy_vis= - alpha*vy(i)
                         
                        
                        !etax = gasdev(idum)*T_eta
                        etay = T_eta*gasdev(idum)
                        etax = T_eta*gasdev(idum)
                        
                        vx(i)=vx(i)+(fx_vis+fx_int(i))*dt+etax
                        vy(i)=vy(i)+(fy_vis+fy_int(i))*dt+etay
                        x(i)= x(i) + vx(i)*dt
                        y(i)= y(i) + vy(i)*dt
